...
public final class ListsByIndexTest extends BaseByIndexTest {
    ...
    protected <T, R> R[] map(T[] array, IntFunction<R[]> createMappedArray, Function<T, R> mapper) {
        ...
        return Lists.map(Arrays.asList(array), mapper).toArray(createMappedArray.apply(array.length));
    }

    @Override
    protected <T> boolean equals(T[] array1, int startIndex1, T[] array2, int startIndex2, int numElements) {

        return Lists.equals(null, startIndex1, null, startIndex2, numElements);
    }
