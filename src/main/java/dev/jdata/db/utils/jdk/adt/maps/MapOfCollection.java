...
public abstract class MapOfCollection<K, V, C extends Collection<V>, M extends Map<K, C>> implements IMapOfJDKCollection<K, V, C> {
    ...
    protected MapOfCollection(int initialCapacity, int collectionInitialCapacity, IntFunction<M> createMap) {
        ...
        Checks.isIntInitialCapacityAtOrAboveZero(collectionInitialCapacity);
    ...
    @Override
    public final long keys(IObjectAnyOrderAddable<K> addable) {

        Objects.requireNonNull(addable);

        for (K key : map.keySet()) {

            addable.addInAnyOrder(key);
        }
