...
abstract class BaseLongBucketLargeSet
        ...
        extends BaseBucketLargeSet<IMutableLongLargeArray, IMutableLongLargeSinglyLinkedMultiHeadNodeList<BaseLongBucketLargeSet>>
        implements IBaseLongSetCommon {

    private static final boolean DEBUG = DebugConstants.DEBUG_BASE_LONG_BUCKET_LARGE_SET;

    private static final Class<?> debugClass = BaseLongBucketLargeSet.class;

    private static final ILongNodeSetter<BaseLongBucketLargeSet> headSetter = DEBUG
    ...
    private static final ILongNodeSetter<BaseLongBucketLargeSet> tailSetter = (i, t) -> { };

    private long scratchHashArrayIndex;
    private IMutableLongLargeArray scratchHashArray;

    BaseLongBucketLargeSet(AllocationType allocationType, int initialOuterCapacityExponent, int capacityExponentIncrease, int innerCapacityExponent, float loadFactor) {
        super(allocationType, initialOuterCapacityExponent, capacityExponentIncrease, innerCapacityExponent, loadFactor, BaseLongBucketLargeSet::createHashArray,
                BaseLongBucketLargeSet::clearHashArray);
    ...
    BaseLongBucketLargeSet(AllocationType allocationType, int initialOuterCapacityExponent, int innerCapacityExponent, long[] values) {
    ...
    private BaseLongBucketLargeSet(AllocationType allocationType, int initialOuterCapacity, int innerCapacityExponent) {
    ...
    public final <P, E extends Exception> void forEach(P parameter, ILongForEach<P, E> forEach) throws E {
        ...
        final IMutableLongLargeSinglyLinkedMultiHeadNodeList<BaseLongBucketLargeSet> buckets = getBuckets();
    ...
    @Override
    final void rehashBuckets(IMutableLongLargeArray hashArray, IMutableLongLargeArray newHashArray, long newKeyMask,
            IMutableLongLargeSinglyLinkedMultiHeadNodeList<BaseLongBucketLargeSet> buckets, IMutableLongLargeSinglyLinkedMultiHeadNodeList<BaseLongBucketLargeSet> newBuckets) {

        if (DEBUG) {

            enter(b -> b.add("hashArray", hashArray.toHexString()).add("newHashArray", newHashArray).hex("newKeyMak", newKeyMask).add("buckets", buckets)
                    .add("newBuckets", newBuckets));
        }
        ...
        for (long i = 0L; i < hashArrayLength; ++ i) {
            ...
            for (long node = bucketHeadNode; node != noNode; node = buckets.getNextNode(node)) {
                ...
                add(newHashArray, newBuckets, buckets.getValue(node), newKeyMask);
            }
        }

        if (DEBUG) {

            exit(b -> b.add("hashArray", hashArray.toHexString()).add("newHashArray", newHashArray).hex("newKeyMak", newKeyMask).add("buckets", buckets)
                    .add("newBuckets", newBuckets));
        }
    }
    ...
    final boolean addValue(long value) {
        ...
        final boolean newAdded = add(getHashed(), getBuckets(), value, keyMask);
    ...
    private boolean add(IMutableLongLargeArray bucketHeadNodeHashArray, IMutableLongLargeSinglyLinkedMultiHeadNodeList<BaseLongBucketLargeSet> buckets, long value, long keyMask) {
    ...
    private static IMutableLongLargeArray createHashArray(int initialOuterCapacity, int innerCapacityExponent) {
        ...
        return IHeapMutableLongLargeArray.create(initialOuterCapacity, innerCapacityExponent, NO_LONG_NODE);
    }
