...
abstract class BaseLongMaxDistanceNonBucketSet extends BaseLongNonBucketSet implements ILongSetCommon {

    private static final boolean DEBUG = DebugConstants.DEBUG_BASE_LONG_MAX_DISTANCE_NON_BUCKET_SET;

    private byte[] maxDistances;

    BaseLongMaxDistanceNonBucketSet(int initialCapacityExponent, float loadFactor) {
        super(initialCapacityExponent, loadFactor);
    }

    BaseLongMaxDistanceNonBucketSet(int initialCapacityExponent, int capacityExponentIncrease, float loadFactor) {
        super(initialCapacityExponent, capacityExponentIncrease, loadFactor);
    }

    BaseLongMaxDistanceNonBucketSet(BaseLongMaxDistanceNonBucketSet toCopy) {
        super(toCopy);
    }
