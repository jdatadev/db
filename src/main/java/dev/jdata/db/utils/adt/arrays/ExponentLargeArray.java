...
public abstract class ExponentLargeArray<O, I> extends BaseOneDimensionalLargeArray<O, I> {

    private static final boolean DEBUG = DebugConstants.DEBUG_EXPONENT_LARGE_ARRAY;

    static <T> T instantiateLargeExponentArray(AllocationType allocationType, AllocationMechanism expectedAllocationMechanism, IOuterInnerInstantiator<T> instantiator) {

        AllocationType.checkIsAllocationMechanism(allocationType, expectedAllocationMechanism);
        Objects.requireNonNull(instantiator);

        return Capacity.instantiateOuterCapacityInnerExponent(allocationType, instantiator);
    }

    static <T> T instantiateLargeExponentArray(AllocationType allocationType, AllocationMechanism expectedAllocationMechanism, long initialCapacity,
        ...
        return Capacity.instantiateOuterCapacityInnerExponent(allocationType, initialCapacity, instantiator);
    }

    private static <T> T instantiateOuterCapacityInnerExponent(AllocationType allocationType, AllocationMechanism expectedAllocationMechanism, int initialOuterCapacity,
            int innerCapacityExponent, IOuterInnerInstantiator<T> instantiator) {

        checkInstantiateParameters(allocationType, expectedAllocationMechanism, initialOuterCapacity, innerCapacityExponent);
        Objects.requireNonNull(instantiator);

        return instantiator.instantiate(allocationType, initialOuterCapacity, innerCapacityExponent);
    }

    static void checkInstantiateParameters(AllocationType allocationType, AllocationMechanism expectedAllocationMechanism, int initialOuterCapacity, int innerCapacityExponent) {
    ...
    ExponentLargeArray(AllocationType allocationType, int initialOuterCapacity, int innerCapacityExponent, int numInitialOuterAllocatedInnerArrays, boolean hasClearValue,
            IntFunction<O> createOuterArray, boolean requiresInnerArrayNumElements) {
        super(allocationType, initialOuterCapacity, CapacityExponents.computeLongCapacityFromExponent(innerCapacityExponent), numInitialOuterAllocatedInnerArrays, hasClearValue,
    ...
    ExponentLargeArray(AllocationType allocationType, ExponentLargeArray<O, I> toCopy, IOuterAndInnerArraysCopier<O> copyOuterAndInnerArrays) {
