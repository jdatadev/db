...
abstract class BaseLargeNodeList<
    ...
    protected static void checkCreateEmpty(AllocationType allocationType, int initialOuterCapacity, int innerCapacity) {

        checkCreate(allocationType, initialOuterCapacity, innerCapacity);
    }

    protected static void checkCreate(AllocationType allocationType, int initialOuterCapacity, int innerCapacity) {
    ...
    static long[] allocateListNodes(int nodeCapacity) {
        ...
        Checks.isIntCapacityAboveZero(nodeCapacity);
    ...
    protected abstract INodeListView createEmpty(AllocationType allocationType, int initialOuterCapacity, int innerCapacity);

    abstract void reallocateOuter(int newOuterCapacity);
    ...
    BaseLargeNodeList(AllocationType allocationType, int initialOuterCapacity, int innerNodeCapacity, ILargeNodeListValuesFactory<TO_ARRAY, VALUES_LIST, VALUES> valuesFactory) {
        ...
        Checks.isIntInitialOuterCapacityAboveZero(initialOuterCapacity);
        Checks.isInnerCapacityAboveZero(innerNodeCapacity);
    ...
    @Override
    public final INodeListView createEmptyWithSameCapacity() {

        return createEmpty(getAllocationType(), getOuterArrayCapacity(), innerArrayCapacity);
    }

    @Override
    public final INodeListView createEmptyWithCapacityExponentIncrease(int capacityExponentIncrease) {

        return createEmpty(getAllocationType(), getOuterArrayCapacity() << capacityExponentIncrease, innerArrayCapacity);
    }

    protected final <T extends BaseLargeNodeList<?, ?, ?>> T instantiateOuterCapacityInnerCapacity(AllocationType allocationType, IOuterInnerInstantiator<T> instantiator) {

        Objects.requireNonNull(allocationType);
        Objects.requireNonNull(instantiator);

        return instantiator.instantiate(allocationType, getOuterArrayCapacity(), innerArrayCapacity);
    }
