...
final class CachedObjectIndexList<T> extends ObjectIndexList<T> implements ICachedIndexList<T> {
    ...
    static <T> CachedObjectIndexList<T> createEmptyValuesInitializable(AllocationType allocationType) {

        AllocationType.checkIsCached(allocationType);

        return new CachedObjectIndexList<>(allocationType);
    }
    ...
    private CachedObjectIndexList(AllocationType allocationType) {
