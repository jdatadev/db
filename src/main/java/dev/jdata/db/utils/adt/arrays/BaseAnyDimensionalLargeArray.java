...
abstract class BaseAnyDimensionalLargeArray<O, I> extends BaseAnyDimensionalArray<Void, Void, Void> {
    ...
    final O reallocateOuterArrayAndInnerArrayNumElementsWithExistingLength(int outerArrayLength) {
        ...
        Checks.isOuterCapacityAboveZero(outerArrayLength);
    ...
    final O reallocateOuterArrayAndInnerArrayNumElements(int newOuterCapacity) {

        Checks.isOuterCapacityAboveZero(newOuterCapacity);

        if (DEBUG) {

            enter(b -> b.add("newCapacity", newOuterCapacity));
        }

        final O result = copyOuterArray(outerArray, newOuterCapacity);
        ...
        if (isNumInnerElementsRequired()) {
            ...
            this.innerArrayNumElements = innerArrayNumElements != null
                    ? Arrays.copyOf(innerArrayNumElements, newOuterCapacity)
                    : new int[newOuterCapacity];
        }
