...
abstract class BaseLongBucketMap<
                LIST extends BaseLargeLongMultiHeadSinglyLinkedList<MAP, LIST, VALUES>,
                VALUES extends BaseLongValues<LIST, VALUES>,
                MAP extends BaseLongBucketMap<LIST, VALUES, MAP>>

        extends BaseIntegerBucketMap<long[], long[], LIST, VALUES, MAP>
        implements ILongKeyMap, ILongContainsKeyMap {

    private static final boolean DEBUG = DebugConstants.DEBUG_BASE_LONG_BUCKET_MAP;

    private static final LongNodeSetter<BaseLongBucketMap<?, ?, ?>> headSetter = (i, h) -> i.scratchHashArray[i.scratchHashArrayIndex] = h;
    private static final LongNodeSetter<BaseLongBucketMap<?, ?, ?>> tailSetter = (i, t) -> { };
    ...
    BaseLongBucketMap(int initialCapacityExponent, BiIntToObjectFunction<LIST> createBuckets, BaseValuesFactory<long[], LIST, VALUES> valuesFactory) {
        this(initialCapacityExponent, HashedConstants.DEFAULT_LOAD_FACTOR, createBuckets, valuesFactory);
    }

    BaseLongBucketMap(int initialCapacityExponent, float loadFactor, BiIntToObjectFunction<LIST> createBuckets, BaseValuesFactory<long[], LIST, VALUES> valuesFactory) {
    ...
    BaseLongBucketMap(int initialCapacityExponent, int capacityExponentIncrease, float loadFactor, BiIntToObjectFunction<LIST> createBuckets,
    ...
    private BaseLongBucketMap(int initialCapacityExponent, int capacityExponentIncrease, float loadFactor, int bucketsInnerCapacityExponent,
            BiIntToObjectFunction<LIST> createBuckets, BaseValuesFactory<long[], LIST, VALUES> valuesFactory) {
        super(initialCapacityExponent, capacityExponentIncrease, loadFactor, bucketsInnerCapacityExponent, long[]::new, BaseLongBucketMap::clearHashArray, createBuckets,
                valuesFactory);
    }
    ...
    final <S, D> void keysAndValues(long[] keysDst, S src, D dst, ValueSetter<S, D> valueSetter) {
