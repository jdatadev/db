...
public class Capacity {
    ...
    private static <T> T instantiateOuterCapacityInnerExponent(long capacity, int innerCapacityExponent, BiIntToObjectFunction<T> instantiator) {
        ...
        Checks.isIntInitialCapacityAtOrAboveZero(capacity);
    ...
    private static <T> T instantiateOuterCapacityInnerExponent(AllocationType allocationType, long capacity, int innerCapacityExponent, IOuterInnerInstantiator<T> instantiator) {
        ...
        Checks.isIntOrLongInitialCapacityAtOrAboveZero(capacity);
        Checks.isIntCapacityExponent(innerCapacityExponent);
        Objects.requireNonNull(instantiator);
    ...
    public static <T> T instantiateOuterExponentInnerExponent(AllocationType allocationType, int capacity, IOuterInnerInstantiator<T> instantiator) {

        return instantiateOuterExponentInnerExponent(allocationType, (long)capacity, instantiator);
    }

    public static <T> T instantiateOuterExponentInnerExponent(AllocationType allocationType, long capacity, IOuterInnerInstantiator<T> instantiator) {

        return instantiateOuterExponentInnerExponent(allocationType, capacity, DEFAULT_INNER_CAPACITY_EXPONENT, instantiator);
    }

    private static <T> T instantiateOuterExponentInnerExponent(AllocationType allocationType, long capacity, int innerCapacityExponent, IOuterInnerInstantiator<T> instantiator) {
        ...
        final int initialOuterCapacityExponent = CapacityExponents.computeLongCapacityExponentForAboveZero(initialOuterCapacity);

        return instantiator.instantiate(allocationType, initialOuterCapacityExponent, innerCapacityExponent);
    }
    ...
    static int computeArrayOuterCapacity(long capacity, int innerCapacity) {
        ...
        Checks.isLongCapacityAtOrAboveZero(capacity);
