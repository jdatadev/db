...
abstract class BaseOneDimensionalArray<T> extends BaseAnyDimensionalArray<T, T, T> implements IOneDimensionalArrayCommon {
    ...
    static void checkCreateParameters(AllocationType allocationType, AllocationMechanism expetedAllocationMechanism, int initialCapacity) {

        checkCreateParameters(allocationType, expetedAllocationMechanism);
        Checks.isIntInitialCapacityAtOrAboveZero(initialCapacity);
    }

    static void checkCreateParameters(AllocationType allocationType, AllocationMechanism expetedAllocationMechanism) {

        AllocationType.checkIsAllocationMechanism(allocationType, expetedAllocationMechanism);
    }

    final void checkReallocateParameters(T elementsArray, int elementsArrayLength, int newCapacity) {

        Objects.requireNonNull(elementsArray);
        Checks.isArrayLength(elementsArrayLength);
        Checks.isAboveZero(elementsArrayLength);
        Checks.isIntCapacityAboveZero(newCapacity);
