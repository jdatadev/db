...
import dev.jdata.db.storage.backend.RowDataNumBits;
import dev.jdata.db.storage.backend.StorageTableFileSchema;
import dev.jdata.db.storage.backend.StorageTableSchema;
...
public abstract class BaseFileTableDataStorageBackend extends BaseTableDataStorageBackend implements TableDataStorageBackend {
    ...
    protected BaseFileTableDataStorageBackend(VersionedDatabaseSchemas versionedDatabaseSchemas, TableByIdMap<FileTableStorageFiles> fileTableStorageByTableId) {
        super(versionedDatabaseSchemas);
    ...
    private void append(FileTableStorageFile initialFileTableStorageFile, InsertRows rows, byte[] outputRowBuffer) throws IOException {
    ...
    private void updateRows(FileTableStorageFiles fileTableStorageFiles, UpdateRows rows, byte[] outputRowBuffer, ByteBufferAllocator byteBufferAllocator) throws IOException {
    ...
    private void updateRow(FileTableStorageFile fileTableStorageFile, UpdateRows rows, int rowIndex, byte[] outputRowBuffer) throws IOException {
    ...
    private void deleteRows(FileTableStorageFiles fileTableStorageFiles, DeleteRows rows, byte[] outputRowBuffer, ByteBufferAllocator byteBufferAllocator) throws IOException {
    ...
    private void deleteRow(FileTableStorageFile fileTableStorageFile, DeleteRows rows, int rowIndex, byte[] outputRowBuffer) throws IOException {
    ...
    private FileTableStorageFile addFileTableStorageFile(FileTableStorageFile currentFileTableStorageFile, StorageTableFileSchema storageTableFileSchema, long rowId)
        ...
        if (!storageTableFileSchema.getStorageTableSchema().isSameTableAndShemaVersion(currentStorageTableFileSchema.getStorageTableSchema())) {
