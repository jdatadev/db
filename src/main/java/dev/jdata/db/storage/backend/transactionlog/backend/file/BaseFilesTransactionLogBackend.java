...
import dev.jdata.db.storage.backend.StorageTableSchema.StorageSchemaColumn;
...
*/
    ...
    private void insertOrUpdateRows(int tableId, TransactionOperationCode operationCode, InsertUpdateRows<?> rows, ByteBufferAllocator byteBufferAllocator) throws IOException {
        ...
        try {
            ...
            BitBufferUtil.setIntValue(outputBuffer, outputBufferBitOffset, NUM_INT_BITS, tableId, false);
            ...
            for (int i = 0; i < numRows; ++ i) {
                ...
                BitBufferUtil.setLongValue(outputBuffer, outputBufferBitOffset, NUM_LONG_BITS, rows.getRowId(i), false);

                outputBufferBitOffset += NUM_LONG_BITS;

                final long transactionId = rows.getTransactionId(i);

                BitBufferUtil.setLongValue(outputBuffer, outputBufferBitOffset, NUM_LONG_BITS, transactionId, false);
